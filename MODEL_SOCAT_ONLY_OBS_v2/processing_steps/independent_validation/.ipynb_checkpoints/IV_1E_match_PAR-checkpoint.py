...
# === Paths ===
socat_rrs_path = ("/remote/unity/bgc-output/DELAIGUE/SOCAT-FLOATS-OSSE/MODEL_SOCAT_ONLY_OBS_v2/processing_steps/independent_validation/pco2_independent_validation_data_regridded_SLA_RRS.csv")
...
output_path = ("/remote/unity/bgc-output/DELAIGUE/SOCAT-FLOATS-OSSE/MODEL_SOCAT_ONLY_OBS_v2/processing_steps/independent_validation/pco2_independent_validation_data_regridded_SLA_RRS_PAR.csv")

# === Parameters ===
lat_min, lat_max = -90, -30
grid_center_offset = 0.5
lat_center_max = -30.5           # enforce "30S and south" on snapped centers
varname = "PAR_mean"

# ===============================================================
# 1. Load SOCAT–MLD–SLA–RRS dataset
# ===============================================================
print("Loading SOCAT–MLD–SLA–RRS dataset...")
df_all = pd.read_csv(socat_rrs_path)
...
# Enforce Southern Ocean domain on centers
...
def load_par_year(year):
    ...
    for fpath in tqdm(files, desc=f"Loading PAR {year}"):
        try:
            ...
            ym_str = fname.split("_")[1][:6]  # e.g., "199701"
            df_month["month_center"] = pd.to_datetime(ym_str, format="%Y%m")  # month start
    ...
    # Normalize month keys to month START and round coordinates
