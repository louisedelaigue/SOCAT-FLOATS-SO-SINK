...
"""
This script loads the fully merged SOCAT 1°×1° monthly dataset
...
# ===============================================================
# 7. Load ARMOR3D climatology and compute anomalies
# ===============================================================

print("Loading ARMOR3D climatology (1° monthly)...")

clim = pd.read_csv(
    "/home/ldelaigue/Documents/Python/SOCAT-OSSE-SO/"
    "PREDICTION_MATRIX/processing_steps/ARMOR3D_MLD_TS_monthly_climatology_1deg.csv"
)

# Ensure SOCAT df has a "month" column
df["month"] = df["month_center"].dt.month

# Merge using (lat, lon, month)
print("Merging ARMOR3D climatology into SOCAT dataset...")
df = df.merge(
    clim,
    on=["lat_center", "lon_center", "month"],
    how="left",
)

# Compute anomalies
print("Computing anomalies: SOCAT - ARMOR3D climatology...")
df["temperature_anom"] = df["SOCAT_temperature"] - df["ARMOR3D_temperature_clim"]
df["salinity_anom"]    = df["SOCAT_salinity"]    - df["ARMOR3D_salinity_clim"]
