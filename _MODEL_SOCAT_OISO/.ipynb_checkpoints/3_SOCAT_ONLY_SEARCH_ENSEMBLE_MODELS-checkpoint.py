# ===============================================================
# Random Search MLP Training on SOCAT 1×1° Gridded Monthly Data
...
# CSV stats file path (append mode)
stats_path = f"{model_output_dir}/random_search_model_stats.csv"

# If it already exists, delete it (clean fresh run)
...
# ===============================================================
print("\n[INFO] Selected input features:")
print(f"Total = {len(input_columns)} variables\n")
...
# SAVE SCALER in the MODELS folder (updated!)
...
neurons1 = np.arange(75, 91)   # 75–90
neurons2 = np.arange(65, 81)   # 65–80
neurons3 = np.arange(60, 76)   # 60–75
...
#search_space = [(int(rng.choice(neurons1)),
#                 int(rng.choice(neurons2)),
#                 int(rng.choice(neurons3))) for _ in range(200)]

print(f"[INFO] Random search prepared with {len(search_space)} architectures.")

# ===============================================================
# TRAINING LOOP — Random Search
# ===============================================================
for idx, (l1, l2, l3) in enumerate(search_space, start=1):
    ...
    for epoch in range(max_epochs):
        ...
        with torch.no_grad():
            ...
            val_rmse = mean_squared_error(y_val.flatten(), val_pred, squared=False)

        if epoch % 50 == 0:
            print(f"[Epoch {epoch+1}] TrainLoss={epoch_loss:.4f} | ValRMSE={val_rmse:.4f}")
    ...
    with torch.no_grad():
        ...
        final_rmse = mean_squared_error(y_val.flatten(), final_pred, squared=False)
    ...
    with torch.no_grad():
        ...
        test_rmse = mean_squared_error(y_test.flatten(), test_pred, squared=False)
    ...
    # -----------------------------------------------------------
    # APPEND METRICS TO CSV (✓ requested change)
    # -----------------------------------------------------------
    row = pd.DataFrame([{
        "model": idx,
        "layer1": l1,
