...
Steps:
 1. Regrid Copernicus SLA (0.125° daily) to 1° monthly (median per cell)
 2. Merge SLA with the SOCAT–MLD grid
"""
...
# === Paths ===
socat_mld_path = "/home/ldelaigue/Documents/Python/SOCAT-OSSE-SO/MODEL_SOCAT_ONLY_OBS_v2/processing_steps/SOCATv2025_SO_clean_MLD_monthly_regridded.csv"
output_path = "/home/ldelaigue/Documents/Python/SOCAT-OSSE-SO/MODEL_SOCAT_ONLY_OBS_v2/processing_steps/SOCATv2025_SO_clean_MLD_SLA_monthly_1deg.csv"
...
print(f"Loaded {len(df_socat_mld):,} SOCAT–MLD grid cells "
...
for year in years:
    ...
    print("[INFO] Regridding SLA to 1° monthly grid...")
    ds_monthly = ds[variable].resample(time="1ME").mean(skipna=True)
    ...
    # Aggregate SLA to monthly × 1° cells (median)
    sla_monthly = (
...
print(f"\nTotal SLA grid cells after regridding: {len(df_sla):,}")

# Align coordinates and timestamps
...
for df in [df_sla, df_socat_mld]:
    ...
    df["month_center"] = pd.to_datetime(df["month_center"]).dt.to_period("M").dt.to_timestamp(how="end")
...
# Merge SLA with SOCAT–MLD (inner join)
df_merged = pd.merge(
    df_socat_mld,
    df_sla,
...
print(f"\nMerged SOCAT–MLD–SLA dataset: {len(df_merged):,} matched cells "
      f"({len(df_merged) / len(df_socat_mld) * 100:.2f}% coverage).")
